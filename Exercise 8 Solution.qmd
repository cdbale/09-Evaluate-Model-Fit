...
Suppose, hypothetically, that soup `sales` are determined entirely by `discount`, the amount (in cents) that the price of a product is discounted, and `coupon`, whether consumers were sent a digital coupon for a buy-one, get-one free deal. Kroger is interested in evaluating whether the discount or the coupon had a larger effect on sales.
...
Both models are able to recover parameters. The incorrect model that includes the superfluous `coupon` explanatory variable also correctly identifies the variable as not statistically significant (i.e., the confidence interval *includes* zero). In other words, the model identifies that the parameter estimate for `coupon` isn't any different from zero -- meaning the variable has no apparent effect on the outcome.

As a bonus, see that the p-value is huge, much larger than the 0.05 cutoff, which says it's not statistically significant.

Ultimately, it is likely that Kroger's model identified no statistically significant effect of coupons on sales, which may be why the issue was identified in the first place.
